function [u, v, a_t, fs_st, fs, T, phi, M, K, C, time, ug_up] = MDOF_Shear_IMK_seismic_CDM...
 (h, wi, Pi, k, Xi, ug, dt, do, Vo, Fy, a_s, dcdy, a_c, g, Name, StrengthLimitCheck,CMBt,ij,zeta)
%% 显式中心差分法法求解函数
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%by郭豪鑫%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%--------------------------------------------------------------------------
...
fs(:,2) = [-diff(fs_st(:,1));fs_st(end,1)]; % diff 前后两个元素的差值 用来求层间恢复力
...
u(:,2) = do;                % Initial Displacement   初始位移
v(:,2) = Vo;                % Initial Velocity       初始速度
a(:,2) = M\(P(:,1)-C*v(:,2)-fs(:,1));  % Initial Relative Acceleration 初始相对加速度
%% dt-1时刻的相对加速度
u(:,1)=u(:,2)-dt*v(:,2)+(dt^2)/2*a(:,2);
aa1 = 1/(dt^2);aa2 = 1/(2*dt);aa3 = 2*aa1;
aa4 = aa1*M-aa2*C; aa5= -aa3*M;
aa6 = aa1*M + aa2*C;
%% 循环计算
for i= 2: size(u,2)-1
    fs(:,i) = [-diff(fs_st(:,i));fs_st(end,i)];          % 求解层间恢复力
    p_ = P(:,i) - aa4 *u(:,i-1) - aa5*u(:,i)-fs(:,i);
    u(:,i+1)=aa6\p_;
    v(:,i+1) = (u(:,i+1)-u(:,i-1))/2/dt  ;               % Rel Velocity
    a(:,i+1) = 1/(dt^2) * (u(:,i+1)-2*u(:,i)+u(:,i-1));  % Relative acceleration
    if   CMBt == 2
