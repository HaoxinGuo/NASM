function [u, v, a_t, fs_st, fs, T, phi, M, K, C, time, ug_up] = MDOF_Shear_IMK_seismic_GuiYao...
...
%%  参数
lambda=4;
alpha = (2*lambda*M+lambda*dt*C+2*dt*dt*K)\(2*lambda*M);
%% 循环计算
for i= 1: size(u,2)-1
    v(:,i+1)= v(:,i) + dt*alpha*a(:,i);                                %下一时刻速度 % Rel Velocity
    u(:,i+1) = u(:,i) + dt* v(:,i) + dt * dt *alpha * a(:,i);                          %下一时刻位移
    if   CMBt == 2
    ...
    Ci = C*v(:,i+1);
    a(:,i+1) = M\(P(:,i+1)-fs(:,i+1)-Ci);           % Relative acceleration
end
