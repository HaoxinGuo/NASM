function [u, v, a_t, fs_st, fs, T, phi, M, K, C, time, ug_up] = MDOF_Shear_IMK_seismic_SEEA...
...
beta1 = Alpha\((C*dt*gamma)+M);
beta2 = (Alpha\(M+(gamma-2*beta)*C*dt))/2;
beta3 = Alpha\(alpha3*(M+gamma*dt*C)+alpha4*beta*dt*dt*K);
M1=M*(I-beta3)+(1-alpha4)*gamma*dt*C;
M2=M*beta3+(1-alpha4)*(1-gamma)*dt*C;
%% 循环计算
for i= 1: size(u,2)-1
%     v(:,i+1) =  v(:,i) +dt*beta1*a(:,i);                                   %下一时刻速度 % Rel Velocity
    u(:,i+1)=u(:,i)+dt*beta1*v(:,i)+dt*dt*beta2*a(:,i);                    %下一时刻位移  ;                    
    ...
    a(:,i+1)=M1\(p_-fi-C*v(:,i)-M2*a(:,i));                                  %  Relative acceleration     
    v(:,i+1) = v(:,i)+dt*((1-gamma)*a(:,i)+gamma*a(:,i+1));      % 下一时刻速度 % Rel Velocity     
end
